...
int CMyExcel::writeMatrix(const string &matName ,double *mateData,long row,long col)
{
	...
	CString mCol(_T("col="));
	CString temp;
	temp.Format(_T("%ld"),row); //将long型的变量row转换成字符赋值给temp
	mRow = mRow + temp;
	temp.Empty();               //赋值空字符
	temp.Format(_T("%ld"),col); //将long型的变量col转换成字符赋值给temp
	mCol = mCol + temp;
	rowName(locPst,matrixCt);
	position0 = locPst;
	temp.Empty();               //赋值空字符
	temp.Format(_T("%ld"),m_rowCount++);//将long型的变量m_rowCount转换成字符赋值给temp
	position0+=temp;
