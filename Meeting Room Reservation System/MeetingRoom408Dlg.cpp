...
void CMeetingRoom408Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//处理控件IDC_MEETINGROOM408INFO_LIST和变量m_MeetingRoom408InfoList之间的数据交换
...
BOOL CMeetingRoom408Dlg::OnInitDialog()
{
	...
	m_MeetingRoom408InfoList.InsertColumn(0, _T("时间"), LVCFMT_CENTER, 80, 0);
	for(int i = 1;i<m_row;i++)//循环7次给列表每一列的表头赋值，共增加7列
	{
		strYear = m_CurTime.Format("%Y");  //将年转换为字符串
		strMonth = m_CurTime.Format("%m"); //将月转换为字符串
		strDay = m_CurTime.Format("%d");   //将天转换为字符串
		switch(m_CurTime.GetDayOfWeek())//获取周几
		{
	...
	for(int i = 0;i < m_col;i++)
	{
		...
		m_MeetingRoom408InfoList.InsertItem(i, str);//第i行第1列
	}

	UpdateMeetingRoom408();//刷新列表信息
...
void CMeetingRoom408Dlg::UpdateMeetingRoom408()
{
	...
	for(int i=0; i<m_col;i++)
	{
		for(int j=1; j<m_row; j++)
		{
			...
			m_MeetingRoom408InfoList.SetItemText(i, j, str);//循环给每一个表格赋值,第i行第j列
		}
	}
}
